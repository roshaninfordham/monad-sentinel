...
const stages = ["Observed", "Signed", "Batched", "Submitted", "Committed", "Verified"];
...
export function ConfirmationProgress() {
  ...
  const hasRisk = devices.some((device) => device.riskScore >= 70);
  const hasBatch = batches.length > 0;
  const activeIndex = hasBatch ? 5 : hasRisk ? 2 : hasDevices ? 1 : 0;

  return (
    <div className="command-panel rounded-lg px-4 py-3">
      <div className="mb-3 flex items-center justify-between">
        <div className="text-sm font-semibold">Evidence Confirmation</div>
        <div className="text-xs text-[var(--text-secondary)]">Live → Signed → Monad receipt</div>
      </div>
